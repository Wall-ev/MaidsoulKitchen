...
public class FruitBlockDetector {
    // ========== 核心常量（根据MC版本调整） ==========
    // canSurvive方法签名
    ...
    private ClassReader getClassReader(String className) throws IOException {
        ...
        if (is == null) {
            is = FruitBlockDetector.class.getClassLoader().getResourceAsStream(resourcePath);
        }
        if (is == null) {
    ...
    private static class CanSurviveMethodVisitor extends ClassVisitor {
        ...
        public MethodVisitor visitMethod(int access, String name, String desc, String signature, String[] exceptions) {
            ...
            if (CAN_SURVIVE_NAME.equals(name) && CAN_SURVIVE_DESC.equals(desc)) {
                ...
                CanSurviveInsnVisitor insnVisitor = new CanSurviveInsnVisitor(Opcodes.ASM9, mv, access, name, desc);
//                // 持有InsnVisitor引用，用于获取最终检测结果
//                this.isFruitBlockLogic = insnVisitor.isFruitBlockLogic();
                return insnVisitor;
            }
        ...
        public boolean isFruitBlockLogic() {
            return isFruitBlockLogic;
        }

        /**
         * 指令解析器：追踪canSurvive方法内的关键调用（严格校验执行顺序）
         */
        private class CanSurviveInsnVisitor extends AdviceAdapter {
            // 栈状态追踪：记录最近一次调用的方法类型
            private enum StackState {
                NONE,            // 初始状态
                BLOCK_POS_ABOVE, // 刚调用完BlockPos.above()
                GET_BLOCK_STATE  // 刚调用完LevelReader.getBlockState(above的结果)
            }

            private StackState currentStackState = StackState.NONE;
            private boolean matchedFruitLogic = false;
            ...
            public boolean isFruitBlockLogic() {
                return matchedFruitLogic;
            }

            /**
             * 拦截方法调用指令（核心：严格校验调用顺序和栈依赖）
             */
            @Override
            public void visitMethodInsn(int opcode, String owner, String name, String desc, boolean itf) {
                super.visitMethodInsn(opcode, owner, name, desc, itf);

                // ========== 步骤1：检测BlockPos.above()调用 ==========
                if (BLOCK_POS_OWNER.equals(owner) && BLOCK_POS_ABOVE_NAME.equals(name)) {
                    if (BLOCK_POS_ABOVE_DESC_EMPTY.equals(desc) || BLOCK_POS_ABOVE_DESC_INT.equals(desc)) {
                        // 调用完above()，栈顶是BlockPos对象，标记状态
                        currentStackState = StackState.BLOCK_POS_ABOVE;
                        return; // 后续逻辑不执行，等待下一个方法调用
                    }
                }

                // ========== 步骤2：检测LevelReader.getBlockState(above的结果) ==========
                if (currentStackState == StackState.BLOCK_POS_ABOVE) {
                    if (GET_BLOCK_STATE_OWNER.equals(owner) && GET_BLOCK_STATE_NAME.equals(name) && GET_BLOCK_STATE_DESC.equals(desc)) {
                        // getBlockState的参数是栈顶的above()结果，调用后栈顶是BlockState对象
                        currentStackState = StackState.GET_BLOCK_STATE;
                        return; // 后续逻辑不执行，等待下一个方法调用
                    } else {
                        // 栈顶的above()结果未传给getBlockState，重置状态
                        currentStackState = StackState.NONE;
                    }
                }

                // ========== 步骤3：检测BlockState.is(Tag/HolderSet) ==========
                if (currentStackState == StackState.GET_BLOCK_STATE) {
                    if (BLOCK_STATE_OWNER.equals(owner) && BLOCK_STATE_IS_NAME.equals(name)) {
                        // 校验is方法的参数是否符合要求
                        if (isIsMethodWithTagOrHolderSet(desc)) {
                            // 完全匹配：above() → getBlockState() → is(Tag)
                            matchedFruitLogic = true;
                            currentStackState = StackState.NONE; // 匹配完成，重置状态
                            isFruitBlockLogic = true;
                            return;
                        }
                    }
                    // 栈顶的getBlockState结果未调用符合要求的is方法，重置状态
                    currentStackState = StackState.NONE;
                }

                // 其他方法调用，重置状态
                if (!matchedFruitLogic) {
                    currentStackState = StackState.NONE;
                }
            }
...
//                String firstArgDesc = argTypes[0].getDescriptor();
//                // 情况1：is(TagKey<Block>)
//                if (firstArgDesc.startsWith(TAG_KEY_DESC) && firstArgDesc.contains("Block")) {
//                    return true;
//                }
//                // 情况2：is(HolderSet<Block>)
//                if (firstArgDesc.startsWith(HOLDER_SET_DESC) && firstArgDesc.contains("Block")) {
//                    return true;
//                }
//                // 情况3：is(TagKey<Block>, Predicate<BlockState>)
//                if (argTypes.length >= 2) {
//                    String secondArgDesc = argTypes[1].getDescriptor();
//                    return firstArgDesc.startsWith(TAG_KEY_DESC)
//                            && firstArgDesc.contains("Block")
//                            && secondArgDesc.startsWith(PREDICATE_DESC)
//                            && secondArgDesc.contains("BlockState");
//                }
//
//                return false;
//            }
