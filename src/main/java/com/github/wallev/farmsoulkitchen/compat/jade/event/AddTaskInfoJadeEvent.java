package com.github.wallev.farmsoulkitchen.compat.jade.event;
...
public class AddTaskInfoJadeEvent {
    ...
    public void addTaskInfo(AddJadeInfoEvent event) {
        ...
        if (farmTask.getUid().equals(TaskInfo.FRUIT_FARM.uid)) {
            ...
            tooltip.add(Component.translatable("top.farmsoulkitchen.entity_maid.farm.fruit.search_y_offset").append(Component.literal("" + fruitFarmSearchYOffset)));
        }
        ...
        for (IFarmHandlerManager<?> handler : farmTask.getManagerHandlerValues()) {
            ...
            MutableComponent translatable = Component.translatable("top.farmsoulkitchen.entity_maid.farm.rule");
            if (first) {
                first = false;
