...
public class TaskInfoButton extends NormalTooltipButton {
    ...
    public static List<Component> getDesc(IMaidTask task) {
        ...
        components.add(Component.translatable("gui.farmsoulkitchen.widget.cook_guide.task.desc", task.getName()));
        if (task instanceof ICookTask<?, ?> maidTask) {
            ...
            components.add(Component.translatable("gui.farmsoulkitchen.widget.cook_guide.task.recipe_type", typeString).withStyle(ChatFormatting.DARK_GRAY));
        }
        return components;
    }
}
