package com.github.wallev.maidsoulkitchen.task.cook.candlelight;
...
import com.github.wallev.maidsoulkitchen.task.cook.common.TaskLdContainerCook;
...
//        LOGGER.info("hasInput: {} {}", b, hasInput(inventory));
    ...
    protected boolean beHasFuel(StoveBlockEntity blockEntity) {
        return ((IFuelBe)blockEntity).tlmk$isBurning() || ((IStoveBe)blockEntity).tlmk$getTotalBurnTime(blockEntity.getItem(FUEL_SLOT)) > 0;
    }

    protected boolean maidHasFuel(CombinedInvWrapper availableInv, StoveBlockEntity blockEntity) {
       return ItemsUtil.findStackSlot(availableInv, itemStack -> ((IStoveBe)blockEntity).tlmk$getTotalBurnTime(itemStack) > 0) > -1;
    }
    ...
    protected boolean fuelItemAction(EntityMaid entityMaid, StoveBlockEntity blockEntity) {
        ...
        if (((IFuelBe)blockEntity).tlmk$isBurning() || ((IStoveBe)blockEntity).tlmk$getTotalBurnTime(item) > 0) return true;
        ...
        ItemStack fuelStack = ItemsUtil.getStack(availableInv, itemStack -> ((IStoveBe)blockEntity).tlmk$getTotalBurnTime(itemStack) > 0);
