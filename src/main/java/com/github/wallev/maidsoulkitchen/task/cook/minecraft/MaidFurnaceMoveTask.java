...
public class MaidFurnaceMoveTask extends MaidCheckRateTask implements VBehaviorControl {
    private static final int MAX_DELAY_TIME = 120;
    private final float movementSpeed;
    private final int verticalSearchRange;
    private final TaskFurnace task;
    private final AbstractCookingRecipesManager maidRecipesManager;
    protected int verticalSearchStart;

    public MaidFurnaceMoveTask(TaskFurnace task, AbstractCookingRecipesManager maidRecipesManager) {
        this(task, 0.5f, 2, maidRecipesManager);
    }

    public MaidFurnaceMoveTask(TaskFurnace task, float movementSpeed, int verticalSearchRange, AbstractCookingRecipesManager maidRecipesManager) {
    ...
    public AbstractCookingRecipesManager getMaidRecipesManager() {
    ...
    protected boolean shouldMoveTo(ServerLevel worldIn, EntityMaid maid, BlockPos blockPos) {
        ...
        if (this.task.isCookBE(blockEntity)) {
            ...
            return this.task.shouldMoveTo(worldIn, this.maidRecipesManager.getMaid(), (AbstractFurnaceBlockEntity) blockEntity, maidRecipesManager);
        }
        return false;
    }
